...
def mim_cap(
    ...
    bot_pad = pg.rectangle((pad_size[0] + L_overlap, pad_size[1]), layer=layer_set['w1'].gds_layer)
    top_pad = pg.rectangle((pad_size[0] + L_overlap, pad_size[1]), layer=layer_set['w2'].gds_layer)
    b = MIM << bot_pad
    t = MIM << top_pad
...
def transistor(
    ...
    DRAIN = Device("drain")
    LAYOUT = Device("dummy")
    TRANSISTOR = Device(f"TRANSISTOR({L_mesa},{L_gate},{L_overlap},{W_mesa},{W_contact})")

    # create transistor core
    mesa = pg.rectangle((L_mesa, W_mesa), layer=layer_set['mesa'].gds_layer)
    gate = pg.rectangle((L_gate + 2 * L_overlap, W_mesa + 10), layer=layer_set['w1'].gds_layer)
    source = pg.rectangle(((L_mesa - L_gate)/2 + 5, W_contact), layer=layer_set['w2'].gds_layer)
    drain = pg.rectangle(((L_mesa - L_gate)/2 + 5, W_contact), layer=layer_set['w2'].gds_layer)
    ...
    if layer_set['w1'].gds_layer % 2 == 0:
        gate = TRANSISTOR << pg.kl_boolean(A = dev_area, B = gate, operation = 'not', layer=layer_set['w1'].gds_layer)
    else:
        gate = TRANSISTOR << gate
    if layer_set['w2'].gds_layer % 2 == 0:
        source = TRANSISTOR << pg.kl_boolean(A = dev_area, B = source, operation = 'not', layer=layer_set['w2'].gds_layer)
        drain = TRANSISTOR << pg.kl_boolean(A = dev_area, B = drain, operation = 'not', layer=layer_set['w2'].gds_layer)
    else:
        source = TRANSISTOR << source
        drain = TRANSISTOR << drain
    if layer_set['mesa'].gds_layer % 2 == 0:
        mesa = TRANSISTOR << pg.kl_boolean(A = dev_area, B = mesa, operation = 'not', layer=layer_set['mesa'].gds_layer)
    else:
...
def test_chip(
    ...
    ls = LayerSet()
    ls.add_layer(name = 'w1', gds_layer = 1, gds_datatype = 0, description = 'tungsten gate', color = (0.6,0.7,0.9))
    ls.add_layer(name = 'w2', gds_layer = 3, gds_datatype = 0, description = 'tungsten source/drain', color = (0.5,0.4,0.4))
    ls.add_layer(name = 'mesa', gds_layer = 5, gds_datatype = 0, description = 'ito/igzo mesa', color = (0.6,0.2,0.5))
    TOP = Device("top")
    ...
    for i in range(4):
        for li, layer in enumerate(ls._layers.keys()):
            ...
            if i == 0 or i == 3:
                ...
                neg.move(((-1)**(i // 2) * (sample_w / 2 - 1000*li - 500 - alignment_offset), 0))
                pos.move(((-1)**(i // 2) * (sample_w / 2 + 1000*li + 500 - alignment_offset), 0))
            else:
                neg.move(((-1)**(i // 2) * 300, 0))
                pos.move(((-1)**(i // 2) * 300, 0))
                neg.move((0, (-1)**(i // 2) * (sample_w / 2 - 1000*li - 500 - alignment_offset)))
                pos.move((0, (-1)**(i // 2) * (sample_w / 2 + 1000*li + 500 - alignment_offset)))
        a = TOP << align
        rotate_i(a, i)
